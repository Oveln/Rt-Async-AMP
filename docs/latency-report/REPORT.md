...
| **已实施待验收** | **P3 fence 去冗余** | **已落地（ov-channels b45bc0a + ov-rpc stamps 对齐 + 主仓 bd662e7，08-22）**：magic → Relaxed（运行期不变量）+ 自产索引 → Relaxed（SPSC 单写者：try_send 的 write / try_recv·peek·has_pending·is_empty·len 的 read）；**对端索引 Acquire 与发布 Release 原样保留——数据可见性协议零妥协**。首轮板上（itb 586503a5）：真通道路径兑现——`recv_empty` 6627→**2346ns**（3→1 笔 Acquire）、`self_round` 28460→19696（−8.8µs）、`spin_iter` 17997→9436（自旋 6→2 笔）；但 **didx 7.6µs 未动**——归因：ov-rpc stamps 手写展开取包序列（server.rs 逐笔复刻 try_recv）未同步新内存序，已对齐（下一 itb 验收 didx 7.6→~3.2µs）。`recv_seq`/`send_seq` 探针不变是预期（复刻旧序列、硬编码 Acquire，仅作对照）。QEMU 冒烟通过 | **−9~10µs/消息 + 自旋 18→9µs/轮**（didx 对齐后全兑现） | 已完成 / 待验收 |
