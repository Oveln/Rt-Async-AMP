...
**计时源替换候选：首轮受阻，时钟门控假设 + 开门探针**（2026-08-21 板上）：soc-timer counter1（0xd4016000，布局=上游 timer-k1x.c）——**CER bit1 写 8 次回读全 0**、counter1 5ms 零走；d4014000 独立块全零回读。RP 对该块读正常（277ns/笔，不挂死）。两个候选解释：**(a) APBC 时钟门常闭**（主倾向）——主线 ccu-k3.c 给出 `APBC_TIMERS1_CLK_RST @0xd4015044`（bit0=bus gate/bit1=func gate/bit2=reset/mux[6:4]），且 CER=0 连 AP 的 bit0 都是 0 ⇒ **StarryOS 根本不用该块**（走 riscv-timer/Sstc），时钟从未使能——时钟关闭的 APB 块天然"读回 0、写忽略"，完全复现观测；**(b) 跨域访问过滤（读放行/写丢弃）**。裁决探针已备（3447bc2）：`tmr_clkon` 从 RP 写 APBC 0xd4015044 开门+去复位（复位极性双变体自动试）+ 1ms 计数验证——APBC 写粘住且 counter1 起算 ⇒ (a) 成立且块可用（**mux=0 即 12.8MHz，分辨率 78ns**，回卷 335s）；APBC 写也丢 ⇒ (b)，转兜底方案（测量链 mcycle+段间保温，生产链采样制）。
