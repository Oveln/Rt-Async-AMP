...
| **AON_TIMER1（0xc0889000，本地域）** | 待测（tmr_aon 组） | RCPU AON 域专属（06_address_map §6.2），标称 25.6MHz（SEL=0）。**唯一不过簇间互连的候选**——见下方「本地域优先」原则 |
...
| **进行中** | **计时瘦身** | **换源已证伪回滚（6f4ec9d，08-22 板上）**：counter1 间隔读 ~13µs 重锁税（仅轻于 mtime），rtt 240→248.5 净亏。**当前双线**：(a) 采样制/减 mark 笔数（生产链 step/SVC 计时降频或删除；测量链 stamps 采样）——所有**外域**计时器（mtime 24.5µs / counter1 13µs / mcycle 2.9ms）间隔读均有跨域税；(b) **本地域换源**（AON_TIMER1，dfb8b5e）——若 tmr_aon_gapped 实测免重锁税则换源复活，否则 (a) 是唯一出路 | **−20~35µs（D1）**（按保留笔数） | 小 / 低 |
