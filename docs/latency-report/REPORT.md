...
| **AON_TIMER1（0xc0889000，本地域）** | 背靠背 **4ns**；间隔读 **~1.9µs** | RCPU AON 域专属（06_address_map §6.2）。板上验证（08-22）：计数递增、实测 **2.0001MHz**（文档标称 SEL=0=25.6MHz 与实况不符）、热读全系统最快（mtime 106 / mailbox 148 / counter1 277ns）、间隔读税 = mtime 的 1/13（tmr_aon_gapped wall−Σaon 实测，规避插入式盲区）。**生产计时链已迁移（eb2da18）**；分辨率 500ns/tick、回卷 35.8min |
...
| **进行中** | **计时瘦身（本地换源复活）** | counter1 换源证伪回滚（6f4ec9d）后，**本地域 AON_TIMER1 换源达标并已迁移（eb2da18，08-22 板上验证）**：热读 4ns、间隔读税 ~1.9µs（mtime 24.5µs 的 1/13）。预期生产链 2-4 笔 mark 的税 49-98µs → 4-8µs；**待板上 dd 复测确认收益**（若达标即闭环，采样制降级为备选） | **−40~90µs（D1）**（待实测） | 已完成 / 待验证 |
...
**本地域换源：AON_TIMER1 验证达标并迁移（08-22 板上，eb2da18）**：tmr_rt_on 计数递增（Δ=2002/ms）破"块死"旧案；tmr_aon_cal 联标 **2.0001MHz**（文档 SEL=0 标称 25.6MHz 与实况不符，按实测；2MHz 下 32 位回卷 35.8min、分辨率 500ns——dd 分段数十 µs 足够）；**热读 4ns/笔**（全系统最快，快于 mtime 106 / mailbox 寄存器 148 / counter1 277ns——本地域身份的铁证）；**间隔读税 ~1.9µs/笔**（tmr_aon_gapped：wall−Σaon 摊 200 轮，含 mtime 括号冷读摊薄 ~245ns；结构 = 忙等脱离 mtime 计时，规避 counter1 轮的插入式盲区）。比 1µs 判据略超但为 mtime 的 **1/13**——生产链 2-4 笔 mark 的税 49-98µs → 4-8µs。**已迁移**（ISR 戳/T_SCHED/t_seen/step SVC/弹性窗/membench 括号/delay/stamp 链同源；counter1 轮 t_seen 漏迁致混合钟的前车之鉴，本轮全量核查）；mtimecmp 睡眠唤醒仍走 SysTimer。待板上 dd 复测确认收益。副产品：本轮 spin_loop 忙等实测每轮 ~25ms（CVA6 pause 指令比预期贵），不影响判据。
