...
| **✅ 已验收闭环（三代 v2 + dd 回归，08-23 04:06/04:19 轮）** | **A4 缓解（正确性）** | 08-23 01:23 轮：二代看门狗重发被粘滞证伪 → 三代 `cache::republish`（重写同值造新脏行再发布）+ 退避 + 逐轮收口计数（86bc6fe）。04:06 轮验收：fresh_scan 全 8 档 got=1（D=100µs 历史三连丢点转收取），短 D 丢发布为概率性常态、恢复常态在线。v2（10efbe1）封掉 read 字段回卷地雷。**04:19 轮终验：v2 复核 fresh_scan 全绿 + mb 全程无 stray（幻影判据阴性）+ dd 零代价——rtt p50 237.7 ≈ 236.9 基线（send 14.6 同环境态系）、闭环残差 0.0/30、didx 3.1 保持、测量 30 轮零恢复（warmup 首轮一次恢复毫秒级兜住、不进测量面）** | **正确性修复闭环**（产品化收口随 W2 轮询软期限） | 已闭环 |
...
## 附录 A：战役意外修出的三个潜伏 bug（+1 已验收：A4 丢发布三代恢复闭环）
...
4. **定案（08-21 夜）：fresh_scan D=100µs dummy 不可见 = user-cbo `cbo.flush` 静默丢失**（三轮确定性复现）。决定性数据：超时时 **AP 缓存视角发布后 (r=118, w=120)，SRAM/RP 视角 200ms 仍 (119,119)**——AP 的索引行 flush 未把新 write 值写回 SRAM（若写回即使带陈旧 r，RP 也会看到 w=120 出队）。非"同行回卷"分支。**user-cbo 发布链的真实正确性缺口**：publish（fence → cbo.flush → fence）在 X100 U 态存在静默丢失窗口（疑 store 尚在写缓冲时 flush 按行 clean 到的是旧行）。
   ...
   **二代缓解（08-22，b95e950/894f358）= 唯一可信视角是时间**：在途看门狗（bench 侧）——请求发出 3ms（rtt 的 >10 倍余量）未见响应即疑似丢失，幂等重发布（refresh_before_send+publish_send）+ 视 BUSY 补门铃，1ms 连续重试（每次 flush 独立尝试，指数收敛）。安全性：单请求在途协议下，重发时索引行要么干净（重发 no-op）要么脏（发布必未落地 ⇒ RP 未消费 ⇒ 行内 read 与 RP 一致）——二择一，无回卷窗口；多发门铃仅 RP 一次空轮询。恢复线程与主线程发送段互斥（ROUND_LOCK，封闭"已写槽未发布"期间索引被提前冲出的乱序窗口）。
   **二代板上结果（08-23 01:23 轮）→ 粘滞定案**：看门狗机制面正常（mb 全套跑完、生产零命中、无挂死），但主使命失败——**D=100µs 丢发布经 ~200 次 1ms 周期重发仍不落地**（RP 整个 200ms 自旋期全盲）⇒ 丢发布行对 AP 侧重复 CBO 呈**粘滞**，"重试收敛"证伪。关键线索：紧邻 D=300µs（含**新 store** 的正常发送）立即成功 ⇒ 粘滞态绑定旧 store 的行状态，新 store 可破 ⇒ 三代 `cache::republish`（索引重写同值造新脏行再发布）+ 看门狗指数退避（1→64ms，长 op 误报不再刷屏）+ guard 逐轮收口计数（该轮全局限流打印 + 双流交错把 D=100 超时期间的重发证据藏掉，推断而非亲见——收口行直接对账）。
   **三代验收通过（08-23 04:06 轮，86bc6fe + v2 10efbe1）**：fresh_scan **全 8 档 got=1**——D=100µs 历史三连丢点转收取（7.4µs，收口行"恢重发 1 次"证明恢复在位）。更大发现：短 D 档（0/30/100）本轮也都触发了恢复（3/4/1 次）且毫秒级兜住 ⇒ **短 D 丢发布是概率性常态**（此前轮次 0/30 "通过"只是没丢，D=100 的"确定性"是小样本假象），恢复机制常态在线。mb 全套跑完、无空唤醒无 stray 无挂死；开头与退出的 segfault 为已知未修问题（§7）非本轮引入。**v2 回卷地雷修正（同轮复查）**：v1 把 read 字段也按缓存快照重写——干净行场景（mb 类合法长 op 超软期限的误报恢复，该轮 slot=0/7 各 83 次）快照 read 为 RP 消费前陈旧值，重写落地会回卷 RP 消费进度、造幻影消息；该轮恰未显形（疑被 A4 丢失模式挡住，不可依赖）。v2 只快照 AP 自有的 write 字段，read 在 inval 后重读 SRAM 真值——inval 写回滞留脏行或丢弃干净副本两种情况均无回卷窗口。**遗留**：v2 待板上复核（mb 复跑 fresh_scan 应全绿 + 无 stray）；dd 回归确认看门狗零代价（每轮仅 2 笔原子存取 + 收口比较）；W2 轮询产品化时把恢复内建于轮询回路软期限（产品级收口）。
   **v2 复核 + dd 零代价终验（08-23 04:19 轮）——A4 案卷宗**：v2 版 fresh_scan 复跑全 8 档 got=1（0/30/100 档收口 3/4/1 次，同 04:06 形态），mb 全程无 stray（幻影消息判据阴性）；dd n=30 全 D1：**rtt p50 237.7 / mean 238.2**（对照 P3 基线 236.9，send p50 14.6 同环境态系——启动间漂移规则内统计同形）、**闭环残差 0.0/30**、didx 3.1、drx 43.6、S 66.2——看门狗零代价（测量 30 轮零恢复零扰动；warmup rd#1 一次恢复毫秒级兜住，测量面不可见）。**A4 三代弧线（同核检测证伪 → 纯重发粘滞证伪 → 重写同值破粘滞）+ v2 回卷封堵，全闭环**；产品化收口随 W2。开头/退出 segfault 仍为 §7 已知未修问题，与本战役无关。
