...
| ddisp | trap+调度+MSIP 落地 | 27.1 | 含 MSIP 跨桥写落地 ~54µs 物理地板的摊派 + 执行器派发（P2 靶子） |
...
| **P1** | **计时瘦身** | step/SVC/弹性窗计时改采样制或去冷读（生产构建每消息 1-2 笔冷 mtime）；测量构建 stamp 链换源或加保温 | **−35µs（D1）/ −34µs（D2）**，生产构建立即可做 | 小 / 低——纯删减 |
| **P2** | **P3 fence 去冗余** | magic 缓存 + 自产索引 Relaxed + 自旋 6→2 笔（D2 发现粒度 ×3）+ ch2 查询瘦身 | −10µs/消息 + 自旋 18→6µs/轮 | 中 / 低——正确性论证已完成（SPSC 单写者 + RP 无缓存） |
| **P3** | **P2 ISR 直派** | 响应在 ISR 内写完，消灭 MSIP 自环+二次 trap+落地等待 | ddisp 27→~5 | 中 / 中 |
| **P4** | **W2 双向轮询** | AP 响应方向用户态自旋（已实测 −11µs）；绕过 MSIP 54µs 物理地板 | −11µs 起；延迟关键模式更多 | 小（bench 已预演）/ 烧 AP 核 |
| 远期 | 硬件 | mailbox 载荷直传小消息（数据进门铃）/ 硬件 spinlock（0xCAC9_1C00，手册背书） | 再往下 | 大 |

叠加预期：**D1 240→~162，D2 189→~134**。再往下就是 SPSC 协议本体（每消息 4 笔 fence ≈ 8.8µs）+ 数据搬运（~7.5µs）的物理下限区。
