...
| **已实施待验收（三代）** | **A4 缓解（正确性）** | 08-23 01:23 轮：二代看门狗机制面正常（mb 全套跑完无挂死、生产零命中）但 **D=100µs 丢发布经 ~200 次重发仍不落地 ⇒ 粘滞证伪**；线索 = 丢点第三轮仍 exclusive 于 D=100µs + 紧邻 D=300（含新 store）立即成功。三代（86bc6fe）= `cache::republish` 重写同值造新脏行再发布 + 看门狗退避（1→64ms）+ 逐轮收口计数 | **正确性修复**（验收 = fresh_scan D=100µs 转"收取"；若收口行 N>0 仍超时 ⇒ AP 侧恢复死路，转 RP 侧协议方案） | 已完成 / 待板上 |
...
4. **定案（08-21 夜）：fresh_scan D=100µs dummy 不可见 = user-cbo `cbo.flush` 静默丢失**（三轮确定性复现）。决定性数据：超时时 **AP 缓存视角发布后 (r=118, w=120)，SRAM/RP 视角 200ms 仍 (119,119)**——AP 的索引行 flush 未把新 write 值写回 SRAM（若写回即使带陈旧 r，RP 也会看到 w=120 出队）。非"同行回卷"分支。**user-cbo 发布链的真实正确性缺口**：publish（fence → cbo.flush → fence）在 X100 U 态存在静默丢失窗口（疑 store 尚在写缓冲时 flush 按行 clean 到的是旧行）。
   ...
   **二代板上结果（08-23 01:23 轮）→ 三代（08-23，86bc6fe）**：看门狗机制面工作正常（mb 全套 35 op 跑完、生产路径本轮零命中、无挂死），但**主使命失败——D=100µs 丢发布在约 200 次 1ms 周期重发下仍未落地**（RP 整个 200ms 自旋期全盲，超时快照 r=122/w=122 而 AP ③发布后 w=123）⇒ **丢发布行对 AP 侧重复 CBO 呈粘滞，"重试收敛"证伪**。两条关键线索：(1) 丢点第三轮仍 exclusive 于 D=100µs（8 个 D 档仅它丢：0/30/300/1k/3k/10k/50k 全 got=1）；(2) 紧邻的 D=300µs（含**新 store** 的正常发送）立刻成功 ⇒ 粘滞态疑绑定"该笔旧 store 留下的行状态"，新 store 可破。三代恢复 = `cache::republish`：索引字段按缓存视图**重写同值**（造新脏行）→ 标准发布；看门狗节拍改指数退避（1→64ms，合法长 op 不再每毫秒误报——本轮 gapped 计时类 5s op 误触发 ~9000 次），guard Drop 收口打印本轮恢重发次数（本轮全局限流打印把 D=100 超时期间的重发证据藏掉了，stdout/stderr 交错吞行）。**验收判据（三代）**：fresh_scan D=100µs 行转"收取"，或至少超时行后跟随的收口行显示"恢重发 N 次"仍超时（N>0 ⇒ 粘滞对新 store 亦成立，AP 侧恢复彻底死路，转协议级 RP 侧方案）+ mb/dd 长跑无空唤醒挂死。W2 轮询产品化时把恢复内建于轮询回路软期限（产品级收口）。
