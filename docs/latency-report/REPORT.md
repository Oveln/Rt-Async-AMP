...
| **✅ 已验收（三代，08-23 04:06 轮）** | **A4 缓解（正确性）** | 08-23 01:23 轮：二代看门狗重发被粘滞证伪 → 三代 `cache::republish`（重写同值造新脏行再发布）+ 退避 + 逐轮收口计数（86bc6fe）。**04:06 轮验收通过：fresh_scan 全 8 档 got=1（D=100µs 历史三连丢点转收取，7.4µs）**；短 D 档（0/30/100）均触发恢复（3/4/1 次）且毫秒级兜住——短 D 丢发布为概率性常态、非 D=100 独有，此前轮次 0/30 "通过"只是没丢。mb 全套跑完无挂死无 stray。**v2 修正（10efbe1）**：v1 把 read 字段也按缓存快照重写，干净行误报恢复场景会回卷 RP 消费进度（该轮恰未显形）；v2 read 改取 inval 后 SRAM 真值，无回卷窗口 | **正确性修复闭环**（剩：dd 回归确认零代价 + W2 产品化收口） | 主体闭环 / v2 待板上复核 |
