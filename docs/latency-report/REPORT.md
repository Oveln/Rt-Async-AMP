...
**本地域换源：探针达标 → 迁移 → 生产二度证伪回滚（08-22 完整弧线）**：tmr_rt_on 计数递增（Δ=2002/ms）破"块死"旧案；tmr_aon_cal 联标 **2.0001MHz**（文档 SEL=0 标称 25.6MHz 与实况不符）；热读 **4ns/笔**（全系统最快）；纯本地间隔读税 **~1.9µs**（tmr_aon_gapped wall−Σaon）。据此迁移（eb2da18，含 counter1 轮 t_seen 漏迁教训的全量同钟核查，闭环残差 0.0）——但 dd 复测 **rtt 240→251.5 反向**，svc 148/dslot 40.5 与 counter1 轮（146/40.4）几乎同形。**定案：mtime 的 24.5µs 冷读税在生产路径从未被支付**——stamp 点前后交织的 SHM/fence 流量把 mtime 路径保温（"异设备读令 mtime 变热"效应的另一面），基线无税可省，预期 −40~90µs 的前提错误；替换计时器（无论 counter1 还是本地域 AON）在真实间隔（跨域流量填充）下读仍付 ~10µs 级。**探针税 ≠ 生产税**——继插入式盲区后第二个探针-生产外推失效案例，固化规则：**任何计时/访存优化必须以生产路径 dd 复测为准，探针只作筛选**。已回滚（05691b0），timer_k3 驱动与 tmr_aon_* 探针保留。计时链维持 mtime；计时瘦身剩余可行项 = 减 mark 笔数（收益需重估，基线本就无税）。另：send 14.6 连续两轮 vs 基线 8.5（AP 路径未改）——非轮间漂移，疑启动间环境态，挂账。
