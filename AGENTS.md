...
`MUX_MODE4` 等，定义在 `its/k3-pinctrl.h`）。编译链与 esos 一致：
...
2. `Board::init()` → `clock::early_init()`（SPL 握手 + 时钟链）→ `init_dtb`（内嵌
   DTB）→ `DRIVERS.set(K3_DRIVERS)` + `boot()`。
3. `boot()` DFS 先序遍历设备树，对每个节点：先 `try_pinctrl().apply(node)` 应用
   `pinctrl-0`，再按 compatible 匹配 driver 调 `probe()`。
